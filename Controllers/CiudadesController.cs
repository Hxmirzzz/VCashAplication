...
namespace VCashApp.Controllers
{
    public class CiudadesController : BaseController
    {
        ...
        private async Task SetCommonViewBagsAsync(ApplicationUser currentUser, string pageName)
        {
            ...
            bool hasCTYView = await HasPermisionForView(userRoles, "CTY", PermissionType.View);
            bool hasCTYCreate = await HasPermisionForView(userRoles, "CTY", PermissionType.Create);
            bool hasCTYEdit = await HasPermisionForView(userRoles, "CTY", PermissionType.Edit);
            //bool hasSUCDelete = await HasPermisionForView(userRoles, "SUC", PermissionType.Delete);


            ViewBag.canCreate = (bool)ViewBag.IsAdmin || hasCTYCreate;
            ViewBag.canEdit = (bool)ViewBag.IsAdmin || hasCTYEdit;
            //ViewBag.canDeleteSucursal = isAdmin || hasSUCDelete;
        }

        // GET: Ciudades/Index
        [HttpGet]
        [RequiredPermission(PermissionType.View, "CTY")]
        public async Task<IActionResult> Index(int? page, int pageSize = 15, string search = "", bool? estado = null)
        {
            ...
            using (LogContext.PushProperty("IpAddress", ipAddress))
            {
                ...
                await SetCommonViewBagsAsync(currentUser, "Ciudades");

                IQueryable<AdmCiudad> query = _context.AdmCiudades
                ...
                if (!string.IsNullOrEmpty(search))
                {
                    ...
                    query = query.Where(s => s.NombreCiudad.ToLower().Contains(trimmedSearch) ||
                ...
                {
                    return PartialView("~/Views/Ciudades/_CiudadesTablePartial.cshtml", data);
                }
